...
アロケーションが手に負えなくなった時、なにかを妥協しなくてはならないでしょう。
...
これを回避するには投げる例外を事前に事前アロケートする必要があります。
...
これは主にコレクトしたオブジェクトでデストラクタを↑こうするためにおこなわれます。
DではGCアロケーテッドオブジェクトのデストラクタは必要ありません(将来の投稿のトピックです)。
...
ほとんどのDプロジェクトはこのアーティクルのトップのポイント#2のシンプルな軽減戦略から始めて、
パフォーマンスに影響するとき、
ところで`@nogc`や`core.memory.GC`を使うようコードを修正することができますし、そうでなければなりません。
